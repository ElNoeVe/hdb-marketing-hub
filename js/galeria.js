...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (modelosData) {
        ...
        renderNearby(modelosData.cercania);
    }
...
function renderModels(models, filter = 'all') {
    ...
    grid.innerHTML = filtered.map((model, i) => {
        // Placeholder gradient as fallback for images
        const gradients = [
...
function renderNearby(cercania) {
    const grid = document.getElementById('nearbyGrid');
    grid.innerHTML = cercania.map(c => `
    <div class="amenity-item">
      <span class="amenity-icon">${c.icon}</span>
      <div>
        <div style="font-weight:500;">${c.nombre}</div>
        <div style="font-size:0.75rem;color:var(--text-muted);">~${c.distancia}</div>
      </div>
    </div>
  `).join('');
}
