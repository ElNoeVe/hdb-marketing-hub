...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Load live inventory from Supabase and update cards
  await loadDisponibilidad();

  // Setup filter tabs
  setupTabs('.tabs', (filter) => {
    renderModels(data.modelos, filter);
    // Re-apply inventory after re-render
    loadDisponibilidad();
  });
...
async function loadGalleryData() {
  try {
    const res = await fetch(BASE_PATH + 'data/modelos.json?v=' + Date.now());
...
async function loadDisponibilidad() {
  ...
  if (!cfg || !cfg.supabaseUrl) {
    // No config — clear all "Cargando..." badges
  ...
  try {
    ...
    console.warn('⚠️ No se pudo cargar disponibilidad de Supabase:', e.message);
    // Hide "Cargando..." badges on failure
    document.querySelectorAll('.disp-badge').forEach(el => { el.style.display = 'none'; });
  }
}

function applyDisponibilidadToCards(rows) {
  // Build lookup map: modelo_id -> unidades_disponibles
  const inventario = {};
  ...
  document.querySelectorAll('[data-model-id]').forEach(card => {
    ...
    if (availBadgeEl) {
      availBadgeEl.className = disponible
        ? 'badge badge-green gallery-avail-badge'
        : 'badge badge-red gallery-avail-badge';
    ...
    if (dispBadge) {
      if (disponible) {
        dispBadge.textContent = `🏠 ${unidades} unidad${unidades === 1 ? '' : 'es'} disponible${unidades === 1 ? '' : 's'}`;
...
function updateFullscreenImage() {
  ...
  // encodeURI instead of encodeURIComponent for the full path
    ...
    return `
      <div class="gallery-card fade-in delay-${(i % 4) + 1}" data-type="${model.tipo}" data-model-id="${model.id}" style="${!model.disponible ? 'opacity:0.75;' : ''}">
        ...
        <div class="gallery-card-body">
          ...
          <div class="gallery-card-label">Desde:</div>
...
function renderAmenities(amenidades) {
  ...
  grid.innerHTML = amenidades.map(a => {
    const encodedImg = a.imagen ? encodeURI(a.imagen) : '';
    return `
