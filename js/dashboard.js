...
async function loadLatestReport() {
  try {
    // Try to load the latest report by date
    const today = new Date().toISOString().split('T')[0];
    const urls = [
      `data/reportes/reporte-${today}.json`,
      'data/reportes/reporte-2026-02-19.json'
    ];
...
function renderSummary(resumen) {
  ...
  if (cards.length >= 4) {
    cards[0].textContent = formatNumber(resumen.impresiones_totales);
    cards[1].textContent = formatNumber(resumen.clics_totales);
    cards[2].textContent = '$' + formatNumber(Math.round(resumen.gasto_total));
    cards[3].textContent = resumen.leads_totales + resumen.mensajes_totales;
  }

  // Update period label
  const periodEl = document.querySelector('.section-header p');
  if (periodEl && reportData) {
    const fecha = new Date(reportData.fecha_generacion).toLocaleDateString('es-MX', {
      day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit'
    });
    periodEl.textContent = `Período: ${reportData.periodo} | Actualizado: ${fecha}`;
  }
}
