// ===================================
// creativos.js — Strategic Optimizer
// ===================================
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (reportData) {
        renderStrategicView(reportData);
    } else {
        document.getElementById('dailyAdviceContainer').innerHTML =
            `<div class="error-state">😕 No se encontraron datos estratégicos recientes.</div>`;
    }
});
...
function renderStrategicView(data) {
    const dailyContainer = document.getElementById('dailyAdviceContainer');
    const tuesdayContainer = document.getElementById('tuesdayWorkshopContainer');
    const tuesdayCard = document.getElementById('tuesdayWorkshopCard');

    // 1. Daily Optimizer
    if (data.analisis_ia) {
        dailyContainer.innerHTML = formatAIResponse(data.analisis_ia);
    } else {
        dailyContainer.innerHTML = `<p style="color:var(--text-muted);">Sin consejos para hoy.</p>`;
    }

    // 2. Tuesday Workshop
    if (data.es_martes && data.taller_creativo) {
        if (tuesdayCard) tuesdayCard.style.display = 'block';
        tuesdayContainer.innerHTML = formatAIResponse(data.taller_creativo);
    } else {
        // If not tuesday or no workshop data, show a placeholder or keep existing disclaimer
        if (tuesdayContainer) {
            tuesdayContainer.innerHTML = `
                <p style="color:var(--text-muted); font-style:italic;">
                    El Taller Creativo Semanal se genera automáticamente todos los martes a las 7:30 AM analizando los mejores anuncios del último mes.
                </p>`;
        }
    }
}
